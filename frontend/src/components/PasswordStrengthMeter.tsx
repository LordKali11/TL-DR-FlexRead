...
import { PasswordStrengthEvaluation } from '../types';

interface PasswordStrengthMeterProps {
  password?: string;
}

export default function PasswordStrengthMeter({ password = '' }: PasswordStrengthMeterProps): React.ReactElement {
  const { fillClass, label } = useMemo<PasswordStrengthEvaluation>(() => {
    if (!password) {
